...
- [x] 주별 / 월별 출석 현황 — 기간 탭 (일별/주간/월별) UI 구현
- [x] 회원별 출석 현황 조회 — 이달 출석 횟수·이행률 테이블 (`GET /api/attendance/member-stats`)
- [x] 장기 미출석 회원 알림 목록 — 기준 일수 필터 (7/14/30/60/90일) (`GET /api/attendance/inactive`)
- [x] 유증(정지) 전용 UI — 유증 목록, 등록 모달, 해제 기능 (`POST/DELETE /api/attendance/freeze`)
- [ ] 루틴 실행 이력 조회 (날짜별 완료·미완료) — `daily_routines` 테이블 연동 필요
